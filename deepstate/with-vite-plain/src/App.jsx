...
import { 
  Home, Settings, LayoutDashboard, Users, FileText, 
  Mail, BarChart, Activity, PieChart, Calendar, 
  CalendarDays, CalendarRange, Shield, ShieldAlert, 
  Edit, Eye, UserCheck, UserMinus, Clock, CircleDot,
  TrendingUp, Menu, X, ChevronDown, ChevronRight, Plus, Search,
  MoreHorizontal
} from 'lucide-react';

import { navigationTree} from "./NavTree";
      ...
      <div className="flex items-center space-x-1 overflow-x-auto px-2 py-1 border-b border-gray-200">
        {navigationTree.main.map(item => {
          ...
          return (
            ...
            >
              {item.iconName && <span className="mr-2">{renderIcon(item.iconName)}</span>}
              <span>{item.label}</span>
            </button>
          );
        })}
        ...
        <div className="flex items-center space-x-8 px-6 overflow-x-auto">
          {activeSubsection.children.map(item => {
            ...
            return (
              ...
              >
                {item.iconName && <span className="mr-2">{renderIcon(item.iconName, "w-4 h-4")}</span>}
                <span>{item.label}</span>
              </button>
            );
          })}
    ...
    return (
      <div className="fixed inset-0 z-50 bg-white">
        ...
        <div className="p-4 overflow-y-auto max-h-screen">
          {/* Main Navigation */}
          <div className="mb-6">
            <h3 className="text-gray-500 text-sm font-medium mb-2">Main Navigation</h3>
          ...
          {activePage.includes('/dashboard') && (
            <div className="mb-6">
              <h3 className="text-gray-500 text-sm font-medium mb-2">Dashboard Views</h3>
              {navigationTree.main.find(item => item.id === 'main')?.children.map(item => (
                <div key={item.id} className="mb-2">
                  <button
                    onClick={() => handleNavClick(item.path)}
                    className={`flex items-center py-2 px-3 rounded-md w-full text-left ${
                      isActive(item) ? 'bg-gray-100 font-medium' : 'hover:bg-gray-50'
                    }`}
                  >
                    {item.iconName && <span className="mr-3">{renderIcon(item.iconName)}</span>}
                    <span>{item.label}</span>
                  </button>
                </div>
              ))}
        ...
        {pathSegments.map((segment, index) => {
          ...
          return (
            <React.Fragment key={index}>
              ...
              <span className={isLast ? "font-medium text-gray-900" : ""}>{formattedSegment}</span>
            </React.Fragment>
          );
        })}
      </div>
    ...
    return (
      <div className="p-6">
        ...
        <p className="text-gray-600 mb-8">
          This is a demo of the Navigator component. The current page is: {activePage}
        </p>
          ...
          <div className="border border-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="flex items-center">12,543 <TrendingUp className="ml-2 text-green-500" size={16} /></span>
              </div>
              <div className="flex justify-between items-center">
                <span>Conversion Rate</span>
                <span className="flex items-center">3.2% <TrendingUp className="ml-2 text-green-500" size={16} /></span>
              </div>
              <div className="flex justify-between items-center">
                <span>Bounce Rate</span>
                <span className="flex items-center">42.1% <TrendingUp className="ml-2 text-red-500 rotate-180" size={16} /></span>
              </div>
            </div>
          </div>
        )}
        ...
        {activePage.includes('/overview') && (
          <div className="border border-gray-200 rounded-lg p-6">
            ...
            <p className="text-gray-500 mb-4">Summary of your dashboard activity</p>
          </div>
        )}
