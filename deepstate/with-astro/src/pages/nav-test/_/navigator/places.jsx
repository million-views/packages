...
export function Places({ logo, useNavigation }) {
  // Access navigation state from context
  const { navigationItems, isItemActive, setActiveItem } = useNavigation();
...
function DesktopNavigation({ items, isItemActive, setActiveItem }) {
  return (
...
function MobileNavigation({ items, isItemActive, setActiveItem, close }) {
  return (
      ...
      <div className="space-y-1 pt-2 pb-3">
        {items.map((item) => (
          <a
            ...
            aria-current={isItemActive(item.name) ? "view" : undefined}
          >
            {item.name}
          </a>
        ))}
