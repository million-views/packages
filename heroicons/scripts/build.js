...
let transform = {
  ...
  preact: async (svg, componentName, format, isDeprecated) => {
    // Generate the React component
    let component = await svgr(svg, { ref: true, titleProp: true }, { componentName })
    
    // Replace React imports with Preact imports
    component = component
      .replace('import * as React from "react"', 'import { forwardRef } from "preact/compat";')
      .replace('React.forwardRef', '/*#__PURE__*/ forwardRef');
      
    // Add a deprecation warning if needed
    if (isDeprecated) {
      /** @type {string[]} */
...
async function buildIcons(package, style, format) {
  // For preact, we only support ESM
  if (package === 'preact' && format !== 'esm') {
    return
  }
  ...
  await Promise.all(
    icons.flatMap(async ({ componentName, svg, isDeprecated }) => {
      ...
      } else if (package === 'preact') {
        types.push(`import { JSX } from 'preact';`)
        if (isDeprecated) {
          types.push(`/** @deprecated */`)
        }
        types.push(`interface SVGProps extends JSX.SVGAttributes<SVGSVGElement> {`)
        types.push(`  title?: string;`)
        types.push(`  titleId?: string;`)
        types.push(`}`)
        types.push(`declare const ${componentName}: preact.ForwardRefComponent<SVGSVGElement, SVGProps>;`)
        types.push(`export default ${componentName};`)
      } else {
 ...
 */
async function buildExports(styles, package) {
  let pkg = {}
  ...
  // For Preact, only provide import, not require
  if (package === 'preact') {
    pkg[`.`] = {
      import: `./index.esm.js`,
    }
  ...
  for (let style of styles) {
    if (package === 'preact') {
      // For Preact, only provide ESM format
      pkg[`./${style}`] = {
...
async function main(package) {
  ...
  // Clean directories
  if (package === 'preact') {
    // For preact, we only need to clean ESM directories
    await Promise.all([
      rimraf(`./${package}/16/solid/esm/*`),
      rimraf(`./${package}/20/solid/esm/*`),
      rimraf(`./${package}/24/outline/esm/*`),
      rimraf(`./${package}/24/solid/esm/*`),
    ])
  } else {
    await Promise.all([
  ...
  // Build icons
  if (package === 'preact') {
    // For preact, we only build ESM format
    await Promise.all([
  ...
  packageJson.exports = await buildExports(['16/solid', '20/solid', '24/outline', '24/solid'], package)
